...
class up(nn.Module):
	...
	def forward(self, upsample, corr):
		upsample = self.up(upsample)
		
		diffY = corr.size()[2] - upsample.size()[2]
		diffX = corr.size()[3] - upsample.size()[3]
		corr = nn.functional.pad(corr, (-diffX//2, -diffX+diffX//2, \
			-diffY//2, -diffY+diffY//2))
		
		x = torch.cat([corr, upsample], dim=1)
		return self.conv(x)
...
class UNet(nn.Module):
	def __init__(self, n_class=2, padding=0):
		...
		self.reduce = nn.Sequential(nn.Conv2d(64,n_class,1,padding=padding),\
			nn.ReLU(),nn.Conv2d(n_class,n_class,1,padding=padding),nn.Sigmoid())
