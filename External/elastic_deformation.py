"""
This code is copied from https://github.com/zomux/deepy/blob/master/deepy/preprocessing/elastic_distortion.py
"""
...
def elastic_distortion(image, kernel_dim=21, sigma=6, alpha=30, negated=True):
    ...
    displacement_field_x = np.array([[np.random.uniform(-1, 1) for x in range(image.shape[0])] \
                            for y in range(image.shape[1])]) * alpha
    displacement_field_y = np.array([[np.random.uniform(-1, 1) for x in range(image.shape[0])] \
                            for y in range(image.shape[1])]) * alpha
